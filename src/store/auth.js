    ...
    actions: {
        async getRegisterData({ commit, dispatch }, payload) {
            ...
            try {
                ...
                const newUserData = {
                    userId: data.localId, fullname: payload.fullname, username: payload.username, email: payload.email,
                };
        ...
        async addNewUser({ commit, state }, payload) {
            try {
                await axios.post(
                    `https://market-place-vintage-default-rtdb.firebaseio.com/user.json?auth=${state.token}`, payload
                );
        ...
        async getUser({ commit }, payload) {
            try {
                const { data } = await axios.get(
                    `https://market-place-vintage-default-rtdb.firebaseio.com/user.json`);
                for (let key in data) {
